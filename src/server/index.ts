...
if (!process.env.DUMMY_DATA) log(`yellow`, `Warning: LIVE_DATA_INTERVAL is not set. Defaulting to "${constants.DEFAULT_DUMMY_DATA}".`);
else if (process.env.DUMMY_DATA !== `true` && process.env.DUMMY_DATA !== `false`) throw new Error(`DUMMY_DATA is not true or false.`);

if (!process.env.LIVE_DATA_INTERVAL) log(`yellow`, `Warning: LIVE_DATA_INTERVAL is not set. Defaulting to "${constants.DEFAULT_LIVE_DATA_INTERVAL}".`);
else if (isNaN(parseInt(process.env.LIVE_DATA_INTERVAL))) throw new Error(`LIVE_DATA_INTERVAL is not a number.`);

if (!process.env.SOCKET_PORT) log(`yellow`, `Warning: SOCKET_PORT is not set. Defaulting to "${constants.DEFAULT_SOCKET_PORT}".`);
else if (isNaN(parseInt(process.env.SOCKET_PORT))) throw new Error(`SOCKET_PORT is not a number.`);

if (!process.env.WEBFRONT_PORT) log(`yellow`, `Warning: WEBFRONT_PORT is not set. Defaulting to "${constants.DEFAULT_WEBFRONT_PORT}".`);
